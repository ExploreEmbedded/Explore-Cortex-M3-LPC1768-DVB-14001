...
const adcChannelConfig_st STR_AdcConfig[C_MaxAdcChannels_U8]=
{
...
void ADC_Init()
{
   ...
   /* Enable CLOCK for to controller */
	...
	var_Pclk_u32 = adc_GetPclk();
	var_Pclk_u32 = M_GetClkDivValue(var_Pclk_u32);

	LPC_ADC->ADCR = ((1<<SBIT_PDN) | var_Pclk_u32);  // no START, no BURST, ADC normal operation, with selected channel
...
uint16_t ADC_GetAdcValue(uint8_t var_adcChannel_u8)
 {
    ...
    if(var_adcChannel_u8 < C_MaxAdcChannels_U8)
    {
       /* Select channel is with range, COnfigure the channel for ADC and DO the A/D conversion */ 
	   GPIO_PinFunction(STR_AdcConfig[var_adcChannel_u8].pinNumber,STR_AdcConfig[var_adcChannel_u8].PinFunSel);
       LPC_ADC->ADCR  = (LPC_ADC->ADCR  & 0xFFFFFF00) | (0x01 << var_adcChannel_u8 );		/* set the channel */
	   ...
	   var_adcResult_u16 = (LPC_ADC->ADGDR >> SBIT_RESULT) & 0xfff; /*Read the 12bit adc result*/
    }
    else
    {
	...
	return(var_adcResult_u16);                        			/* Return the 12-bit result */
 }
...
static uint32_t adc_GetPclk(void)
{
    uint32_t var_AdcPclk_u32,var_Pclk_u32;
	/** 
	...
	var_AdcPclk_u32 = (LPC_SC->PCLKSEL0 >> 6) & 0x03;
	
	switch( var_AdcPclk_u32 )
	{
	      case 0x00:
